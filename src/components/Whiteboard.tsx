  ...
  return (
    <div className="isolate flex -space-x-4 overflow-hidden">
      {users.map((u, i) => {
        ...
        return (
          <span
            className="inline-block h-10 w-10 overflow-hidden rounded-full bg-gray-100 border-2 border-white"
            key={i}
          >
...
function getColorFromStr(str: string) {
  const hash = str.split('').reduce((acc, char) => (acc << 5) - acc + char.charCodeAt(0), 0)
